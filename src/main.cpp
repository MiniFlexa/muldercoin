...
#if defined(NDEBUG)
# error "Muldercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x85e2981975bb8d0fb75142b757c50ff2545482575a7a9618f0e8cec1644c120c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Muldercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Muldercoin Signed Message:\n";
...
{
    // Muldercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Muldercoin: 840k blocks in ??? years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Muldercoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // Muldercoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Muldercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Muldercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe1868cbb786c2d6a705beee050816e06d5b08ed42f0ee1ac4a85816f7c45b4a1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 10/Fbr/2031 - MulderCoin goes to the moon!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b9c8c738bf86884d27c429c21f05f021848073c03fe8486a06a1e79de240dd8b9224bea9add31774491bd2a86487e3785fe8da3e3c061a997a152bf3d1f18753") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1612985895;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086116716;

        if (fTestNet)
        {
            block.nTime    = 1612985884;
            block.nNonce   = 386530452;
        }

	//mine genesisblock if set to true
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6f9a217289c0690e2d5d0438fdde434ef30e91617c1804a1e8da4bc2e28b4aa7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xc3, 0xbf, 0xd9 }; // Muldercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MuldercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MuldercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MuldercoinMiner(CWallet *pwallet)
{
    printf("MuldercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("muldercoin-miner");
    ...
    try { loop {
        ...
        printf("Running MuldercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MuldercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MuldercoinMiner, pwallet));
}
